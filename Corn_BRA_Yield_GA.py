...
def Get_Data_Single(scope: dict, var: str = 'yield', fo = {}):    
    if (var=='yield'):
        df = fu.get_BRA_conab_data(states=['NATIONAL'], product='MILHO', crop='1ª SAFRA', years=scope['years'], cols_subset=['year','Yield'], conab_df=None)
        df = df.set_index('year', drop=False)
        return df

    elif (var=='weights'):        
        return fu.get_BRA_prod_weights(states=list(fo['locations']), product='MILHO', crop='1ª SAFRA', years=scope['years'], conab_df=None)
